...
export default function Sidebar() {
  ...
  return (
    <>
      <>
        {/* Mobile Overlay - more premium blur */}
        {!collapsed && (
          <div
        ...
        <aside
          className={`fixed left-0 top-0 z-[60] flex h-screen flex-col border-r border-border bg-theme-card shadow-2xl md:shadow-none transition-all duration-500 ease-[cubic-bezier(0.16,1,0.3,1)] ${collapsed
            ? "-translate-x-full md:w-[80px] md:translate-x-0"
            : "w-[280px] translate-x-0"
            } ${!collapsed && "md:translate-x-0"}`}
        >
          ...
          <div className={`flex shrink-0 items-center justify-between border-b border-theme p-4 transition-all ${collapsed ? "md:justify-center md:flex-col md:gap-3" : ""}`}>
            <Link href="/dashboard" className={`flex min-w-0 items-center gap-3 ${collapsed ? "md:justify-center" : ""}`} onClick={() => !collapsed && toggle()}>
              ...
              {!collapsed && (
                <div className="min-w-0 animate-fade-in">
                  <span className="truncate font-bold text-theme block text-[15px]">Mashori Farm</span>
                  <p className="truncate text-[10px] text-theme-muted font-medium">Land & Farm System</p>
                </div>
              )}
            </Link>
            ...
            {!collapsed && (
              <button
                onClick={toggle}
                className="md:hidden p-2 rounded-xl bg-theme-track text-theme-muted hover:text-red-500 hover:bg-red-500/10 transition-colors border border-theme"
              >
                <X className="w-5 h-5" />
              </button>
            )}
          ...
          {/* Footer actions */}
          <div className="shrink-0 border-t border-theme p-4 flex flex-col gap-2 pb-safe md:pb-4">
            {collapsed ? (
              <button
                type="button"
                onClick={toggle}
